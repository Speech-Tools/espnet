...
dumpdir=/host_shares/espnet2/transfer/dump
expdir=/host_shares/espnet2/transfer/exp
...
pretrained_model=          # Pre-trained model path for Transfer Learning.
pretrained_lm=             # Pre-trained LM path for Transfer Learning.
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        local/data_transfer.sh ${local_data_opts}
    fi
